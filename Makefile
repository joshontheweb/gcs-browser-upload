...
compile:
	...
	$(BIN)/browserify --standalone upload -v dist/index.js | $(BIN)/derequire | $(BIN)/uglifyjs > dist/bundle.js

compile-watch:
	NODE_ENV=production $(BIN)/babel -w src --out-dir dist --copy-file &
	$(BIN)/watchify --standalone upload -v dist/index.js -o dist/bundle.js
